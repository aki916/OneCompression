...
def quantize_qfna(x, scale, zero, maxq):
    """quantize_qfna function - QFN-a quantization"""
    q = torch.clamp(torch.round(x / scale) + zero, 0, maxq)
    return scale * (q - zero)


def quantize_qfnb(x, scale, maxq):
    """quantize_qfnb function - QFN-b quantization"""
    q = x / scale
...
def quantize_qfnc(x, scale, zero, maxq):
    """quantize_qfnc function - QFN-c quantization"""
...
class QuantizerQFN(nn.Module):
    """Quantizer class for QUIP"""

    def __init__(self, shape=1):
        super(QuantizerQFN, self).__init__()
    ...
    def find_params_qfna(self, x, weight=False):
        ...
        if self.mse:
            ...
            for i in range(int(self.maxshrink * self.grid)):
                ...
                q = quantize_qfna(x, scale1.unsqueeze(1), zero1.unsqueeze(1), self.maxq)
                q -= x
                q.abs_()
                q.pow_(self.norm)
                err = torch.sum(q, 1)
