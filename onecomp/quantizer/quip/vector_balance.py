...
def round_allbal(  # pylint: disable=too-many-positional-arguments
    ...
    for ip in range(npasses):
        ...
        if (w_hat == wr).all():
            logger.debug(f"breaking after {ip+1} greedy passes found fixed point")
            break
        w_hat.copy_(wr)
...
def round_ldl(  # pylint: disable=too-many-positional-arguments
    ...
    for igp in range(n_greedy_passes):
        ...
        wr = torch.clamp(wr, min=0, max=2**nbits - 1)
        if (w_hat == wr).all():
            logger.debug(f"breaking after {igp+1} greedy passes found fixed point")
            break
        w_hat.copy_(wr)
...
def round_ldl_admm(  # pylint: disable=too-many-positional-arguments
    ...
    logger.debug(f"objective before greedy passes: {objective}")
    ...
    for jj in range(n_greedy_passes):
        ...
        for ii in range(d):
            ...
            if (wr == wr_prev).all():
                ...
                break
        objective = ((wr - w) @ H @ (wr - w).T).trace()
        logger.debug(f"objective after {jj+1} greedy passes: {objective}")
        if ii == 0:
            logger.debug(f"triangle-greedy finished after {jj+1} passes")
            break
...
def quantize_weight_vecbal(  # pylint: disable=too-many-positional-arguments
    ...
    if (qfn == "a") and (qmethod == "ldl_gptqequiv"):
        wr_int = round_ldl_gptqequiv((w / scale) + zero, H, nbits=nbits)
        wr_dequant = scale * (wr_int - zero)
        return wr_dequant, wr_int.int(), scale
